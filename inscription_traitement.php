...
if(!empty($_POST['nom']) && !empty($_POST['prenom']) && !empty($_POST['email']) && !empty($_POST['password']) && !empty($_POST['password2'])) {
  ...
  $check =  "SELECT * FROM `utilisateur` WHERE Email = ?";
  ...
  if($row == null) {
    ...
    $insert = "INSERT INTO utilisateur (ID_user, Nom , Prenom, Email , Pass) VALUES (null, '".$nom."','".$prenom."','".$email."','".$passwordhash."')";
    if (mysqli_query($conn, $insert)) {
      echo "Compte enregistré avec succès. Redirection dans 3 secondes...";
